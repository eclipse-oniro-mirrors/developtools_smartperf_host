...
void FtraceEventProcessor::SchedEventInitialization()
{
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_SCHED_SWITCH),
                                  std::bind(&FtraceEventProcessor::SchedSwitch, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_SCHED_BLOCKED_REASON),
                                  std::bind(&FtraceEventProcessor::SchedBlockedReason, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_SCHED_WAKEUP),
                                  std::bind(&FtraceEventProcessor::SchedWakeup, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_SCHED_WAKING),
                                  std::bind(&FtraceEventProcessor::SchedWaking, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_SCHED_WAKEUP_NEW),
                                  std::bind(&FtraceEventProcessor::SchedWakeupNew, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_SCHED_SWITCH_LITE),
                                  std::bind(&FtraceEventProcessor::SchedSwitchLite, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_SCHED_WAKEUP_LITE),
                                  std::bind(&FtraceEventProcessor::SchedWakeupLite, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
}
...
void FtraceEventProcessor::TaskEventInitialization()
{
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_TIMERFD_WAKEUP),
                                  std::bind(&FtraceEventProcessor::TimerfdWakeup, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
}

void FtraceEventProcessor::HmfsEventInitialization()
{
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_HMFS_READ_ENTER),
                                  std::bind(&FtraceEventProcessor::HmfsReadEnter, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_HMFS_READ_EXIT),
                                  std::bind(&FtraceEventProcessor::HmfsReadExit, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_HMFS_WRITE_ENTER),
                                  std::bind(&FtraceEventProcessor::HmfsWriteEnter, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_HMFS_WRITE_EXIT),
                                  std::bind(&FtraceEventProcessor::HmfsWriteExit, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
}

void FtraceEventProcessor::BlockEventInitialization()
{
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_BLOCK_RQ_ISSUE),
                                  std::bind(&FtraceEventProcessor::BlockRqIssue, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
    eventNameToFunctions_.emplace(config_.eventNameMap_.at(TRACE_EVENT_BLOCK_RQ_COMPLETE),
                                  std::bind(&FtraceEventProcessor::BlockRqComplete, this, std::placeholders::_1,
                                            std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
}
...
bool FtraceEventProcessor::SchedSwitch(FtraceEvent &ftraceEvent, uint8_t data[], size_t size, const EventFormat &format)
{
    ...
    if (format.fields.size() == DO_NOT_HAVE_NEXT_INFO_SIZE) {
        schedSwitchMsg->set_next_info(INVALID_UINT64);
    } else {
        schedSwitchMsg->set_next_info(
            FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    }
    return true;
}

bool FtraceEventProcessor::SchedSwitchLite(FtraceEvent &ftraceEvent,
                                           uint8_t data[],
                                           size_t size,
                                           const EventFormat &format)
{
    uint8_t index = 0;
    auto schedSwitchLiteMsg = ftraceEvent.mutable_sched_switch_lite_format();
    schedSwitchLiteMsg->set_prev_pid(FtraceFieldProcessor::HandleIntField<int32_t>(format.fields, index++, data, size));
    schedSwitchLiteMsg->set_prev_prio(
        FtraceFieldProcessor::HandleIntField<int16_t>(format.fields, index++, data, size));
    schedSwitchLiteMsg->set_prev_state(
        FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    schedSwitchLiteMsg->set_next_pid(FtraceFieldProcessor::HandleIntField<int32_t>(format.fields, index++, data, size));
    schedSwitchLiteMsg->set_next_prio(
        FtraceFieldProcessor::HandleIntField<int16_t>(format.fields, index++, data, size));
    schedSwitchLiteMsg->set_next_info(
        FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    return true;
}
...
bool FtraceEventProcessor::SchedWakeupLite(FtraceEvent &ftraceEvent,
                                           uint8_t data[],
                                           size_t size,
                                           const EventFormat &format)
{
    uint8_t index = 0;
    auto schedWakeupLiteMsg = ftraceEvent.mutable_sched_wakeup_lite_format();
    schedWakeupLiteMsg->set_pid(FtraceFieldProcessor::HandleIntField<int32_t>(format.fields, index++, data, size));
    schedWakeupLiteMsg->set_prio(FtraceFieldProcessor::HandleIntField<int16_t>(format.fields, index++, data, size));
    schedWakeupLiteMsg->set_target_cpu(
        FtraceFieldProcessor::HandleIntField<int32_t>(format.fields, index++, data, size));
    return true;
}
...
bool FtraceEventProcessor::TimerfdWakeup(FtraceEvent &ftraceEvent,
                                         uint8_t data[],
                                         size_t size,
                                         const EventFormat &format)
{
    uint8_t index = 0;
    auto timerfdWakeupMsg = ftraceEvent.mutable_timerfd_wakeup_format();
    timerfdWakeupMsg->set_thread_pid(FtraceFieldProcessor::HandleIntField<int32_t>(format.fields, index++, data, size));
    timerfdWakeupMsg->set_thread_tid(FtraceFieldProcessor::HandleIntField<int32_t>(format.fields, index++, data, size));
    timerfdWakeupMsg->set_thread_name(FtraceFieldProcessor::HandleStrField(format.fields, index++, data, size));
    timerfdWakeupMsg->set_interval(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    timerfdWakeupMsg->set_curr_mono(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    timerfdWakeupMsg->set_expire_mono(
        FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    return true;
}

bool FtraceEventProcessor::BlockRqIssue(FtraceEvent &ftraceEvent,
                                        uint8_t data[],
                                        size_t size,
                                        const EventFormat &format)
{
    ...
    msg->set_dev(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_sector(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_nr_sector(FtraceFieldProcessor::HandleIntField<uint32_t>(format.fields, index++, data, size));
    msg->set_bytes(FtraceFieldProcessor::HandleIntField<uint32_t>(format.fields, index++, data, size));
    msg->set_rwbs(FtraceFieldProcessor::HandleStrField(format.fields, index++, data, size));
    msg->set_comm(FtraceFieldProcessor::HandleStrField(format.fields, index++, data, size));
    msg->set_cmd(FtraceFieldProcessor::HandleStrField(format.fields, index++, data, size));
    return true;
}

bool FtraceEventProcessor::BlockRqComplete(FtraceEvent &ftraceEvent,
                                           uint8_t data[],
                                           size_t size,
                                           const EventFormat &format)
{
    ...
    msg->set_sector(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_nr_sector(FtraceFieldProcessor::HandleIntField<uint32_t>(format.fields, index++, data, size));
    msg->set_error(FtraceFieldProcessor::HandleIntField<int32_t>(format.fields, index++, data, size));
    msg->set_rwbs(FtraceFieldProcessor::HandleStrField(format.fields, index++, data, size));
    msg->set_cmd(FtraceFieldProcessor::HandleStrField(format.fields, index++, data, size));
    return true;
}

bool FtraceEventProcessor::HmfsReadEnter(FtraceEvent &ftraceEvent,
                                         uint8_t data[],
                                         size_t size,
                                         const EventFormat &format)
{
    uint8_t index = 0;
    auto msg = ftraceEvent.mutable_hmfs_read_enter_format();
    msg->set_dev(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_ino(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_off(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_size(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_i_size(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_name(FtraceFieldProcessor::HandleStrField(format.fields, index++, data, size));
    return true;
}

bool FtraceEventProcessor::HmfsReadExit(FtraceEvent &ftraceEvent,
                                        uint8_t data[],
                                        size_t size,
                                        const EventFormat &format)
{
    uint8_t index = 0;
    auto msg = ftraceEvent.mutable_hmfs_read_exit_format();
    msg->set_dev(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_ino(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_off(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_size(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_res(FtraceFieldProcessor::HandleIntField<int64_t>(format.fields, index++, data, size));
    return true;
}

bool FtraceEventProcessor::HmfsWriteEnter(FtraceEvent &ftraceEvent,
                                          uint8_t data[],
                                          size_t size,
                                          const EventFormat &format)
{
    uint8_t index = 0;
    auto msg = ftraceEvent.mutable_hmfs_write_enter_format();
    msg->set_dev(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_ino(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_off(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_size(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
...
bool FtraceEventProcessor::HmfsWriteExit(FtraceEvent &ftraceEvent,
                                         uint8_t data[],
                                         size_t size,
                                         const EventFormat &format)
{
    uint8_t index = 0;
    auto msg = ftraceEvent.mutable_hmfs_write_exit_format();
    msg->set_dev(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_ino(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_off(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
    msg->set_size(FtraceFieldProcessor::HandleIntField<uint64_t>(format.fields, index++, data, size));
