...
export function lostFrameSender(tName: String, fName: String, row: TraceRow<LtpoStruct>): Promise<LtpoStruct[]> {
...
function arrayBufferHandler(res: any, len: number): LtpoStruct[] {
  let outArr: LtpoStruct[] = [];
  let startTime = new Float64Array(res.startTime);
  let dur = new Float64Array(res.dur);
  let id = new Uint16Array(res.id);
  let presentId = new Float64Array(res.presentId);
  for (let i = 0; i < len; i++) {
    outArr.push({
      id: id[i],
      dur: dur[i],
      startTime: startTime[i],
      presentId: presentId[i],
    } as unknown as LtpoStruct);
  }
  return outArr;
}
