...
namespace OHOS {
namespace SmartPerf {
double ParseClickCompleteTrace::ParseCompleteTrace(const std::string& fileNamePath)
{
    int conversion = 1000;
...
double ParseClickCompleteTrace::GetLineTime()
{
    ...
    while (getline(infile, line)) {
        appPid = GetPid(line, "pid", appPid);
        startTime = GetStartTime(line, startTime);
        ...
        if (doComposition != std::string::npos) {
            ...
            int endNum = SPUtilesTye::StringToSometype<float>(endTime);
            int endFlagNum = SPUtilesTye::StringToSometype<float>(endTimeFlag);
            int startNum = SPUtilesTye::StringToSometype<float>(startTime);
            ...
            if (SPUtilesTye::StringToSometype<float>(endTimeFlag) == 0) {
                endTimeFlag = endTime;
...
double ParseClickCompleteTrace::GetTime(std::string endTime)
{
    ...
    if (SPUtilesTye::StringToSometype<float>(endTime) == 0 || SPUtilesTye::StringToSometype<float>(startTime) == 0) {
    } else {
        double displayTime = 0.032;
        completeTime = SPUtilesTye::StringToSometype<float>(endTime) -
                        SPUtilesTye::StringToSometype<float>(startTime) + displayTime;
    }
    return completeTime;
}
std::string ParseClickCompleteTrace::GetPid(const std::string& line, const std::string &pn, const std::string &pb)
{
    if (appPidnum == 0) {
...
std::string ParseClickCompleteTrace::GetStartTime(const std::string& line, const std::string &startTimeBefore)
{
