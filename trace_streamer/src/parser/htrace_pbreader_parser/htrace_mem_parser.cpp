...
void HtraceMemParser::SpecialDataAddition(ProtoReader::ProcessMemoryInfo_Reader& processMemoryInfo,
                                          uint64_t timeStamp,
                                          uint32_t ipid,
...
uint32_t HtraceMemParser::ParseSmapsPathTypeByPrefix(bool hasX, const std::string& path, const bool hasAppName) const
{
    if (EndWith(path, ".so")) {
        if (hasX) {
...
uint32_t HtraceMemParser::ParseSmapsPathTypeBySuffix(bool hasX, const std::string& path, const bool hasAppName) const
{
...
uint32_t HtraceMemParser::ParseSmapsBlockType(ProtoReader::SmapsInfo_Reader& smapsInfo) const
{
    ...
    uint32_t detailRet = (smapsInfo, path, hasAppName);
