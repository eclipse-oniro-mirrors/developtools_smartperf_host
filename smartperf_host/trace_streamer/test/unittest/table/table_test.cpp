...
const uint64_t CALLCHAIN_ID = 1;
const uint16_t TYPE = 1;
const uint32_t IPID = 1;
const uint32_t ITID = 1;
const uint64_t START_TS = 1663869124160;
const uint64_t END_TS = 1663869124260;
const uint64_t DUR = 100;
const size_t SIZE = 1;
const int32_t FD = 0;
const uint64_t TIMESTAMP = 1663869124160;
const uint32_t FPS = 1;
const uint64_t CALLCHAIN_ID1 = 2;
const uint16_t TYPE1 = 2;
const uint32_t IPID1 = 2;
const uint32_t ITID1 = 2;
const uint64_t START_TS1 = 1663869124161;
const uint64_t END_TS1 = 1663869124261;
const uint64_t DUR1 = 200;
const size_t SIZE1 = 2;
const int32_t FD1 = 1;
const uint64_t TIMESTAMP1 = 1663869224160;
const uint32_t FPS1 = 2;
...
HWTEST_F(TableTest, CallstackTableTest, TestSize.Level1)
{
    ...
    stream_.traceDataCache_->GetInternalSlicesData()->AppendInternalAsyncSlice(callStackInternalRow1, cookid1,
                                                                               parentId1);
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect, false), 2);
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect1, false), 1);
...
HWTEST_F(TableTest, FileSystemSampleTableTest, TestSize.Level1)
{
    ...
    FileSystemSampleRow fileSystemSampleRow = {
        CALLCHAIN_ID, TYPE, IPID, ITID,   START_TS,      END_TS,         DUR,           returnValue,
        errorCode,    SIZE, FD,   fileId, firstArgument, secondArgument, thirdArgument, fourthArgument};
    stream_.traceDataCache_->GetFileSystemSample()->AppendNewData(fileSystemSampleRow);
    fileSystemSampleRow = {CALLCHAIN_ID1,  TYPE1,           IPID1,          ITID1,          START_TS1, END_TS1,
                           DUR1,           returnValue1,    errorCode1,     SIZE1,          FD1,       fileId1,
...
HWTEST_F(TableTest, NativeHookTableTest, TestSize.Level1)
{
    ...
    NativeHookRow nativeHookRow = {CALLCHAIN_ID, IPID,         ITID,     eventType, subType,
                                   timeStamp,    endTimestamp, duration, addr,      memSize};
    stream_.traceDataCache_->GetNativeHookData()->AppendNewNativeHookData(nativeHookRow);
    nativeHookRow = {callChainId1, ipid1,         itid1,     eventType1, subType1,
                     timestamp1,   endTimestamp1, duration1, addr1,      memSize1};
    stream_.traceDataCache_->GetNativeHookData()->AppendNewNativeHookData(nativeHookRow);
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect, false), 2);
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect1, false), 1);
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect2, false), 1);
...
HWTEST_F(TableTest, PerfFilesTableTest, TestSize.Level1)
{
    ...
    DataIndex filePath = stream_.traceDataCache_->GetDataIndex("filePath");
    DataIndex soName = stream_.traceDataCache_->GetDataIndex("soName");
    ...
    DataIndex filePath1 = stream_.traceDataCache_->GetDataIndex("filePath1");
    DataIndex soName1 = stream_.traceDataCache_->GetDataIndex("soName1");

    stream_.traceDataCache_->GetPerfFilesData()->AppendNewPerfFiles(fileIds, serial, symbols, filePath, soName);
    stream_.traceDataCache_->GetPerfFilesData()->AppendNewPerfFiles(fileIds1, serial1, symbols1, filePath1, soName1);
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect, false), 2);
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect1, false), 1);
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect2, false), 0);
}
...
HWTEST_F(TableTest, PerfSampleTableTest, TestSize.Level1)
{
    ...
    PerfSampleRow perfSampleRow = {sampleId,    timestamp,      tid,   eventCount,
                                   eventTypeId, timestampTrace, cpuId, threadState};
    PerfSampleRow perfSampleRow1 = {sampleId1,    timestamp1,      tid1,   eventCount1,
                                    eventTypeId1, timestampTrace1, cpuId1, threadState1};
...
HWTEST_F(TableTest, ThreadStateTableTest, TestSize.Level1)
{
    ...
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect4, false), 2);
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect5, false), 1);
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect6, false), 1);
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect7, false), 0);
    EXPECT_EQ(stream_.traceDataCache_->SearchDatabase(sqlSelect8, false), 0);
}
/**
