...
import { ChartMode, ChartStruct, draw, setFuncFrame, splitSymbol } from '../../bean/FrameChartStruct';
...
const asyncStack = {
  'libark_jsruntime.so': ['AsyncStackTraceManager::SavePromiseNode'],
  'libtaskpool.z.so': ['Task::Task'],
  'libworker.z.so': ['Worker::Worker'],
  'libffrt.so': ['ffrt_submit_base', 'ffrt::QueueHandler::Dispatch'],
  'libeventhandler.z.so': ['sendEvent', 'DistributeEvent'],
  'libuv.so': ['uv_timer_start', 'uv_queue_work'],
  'libarkweb_engine.so': ['Runtime_DfxPromiseThen', 'Runtime_SetAsyncTraceOfMicrotask'],
}
...
export class FrameChart extends BaseElement {
   ...
   * @returns 是否包含
   */
  private isExpectedFunction(str: string, symbol: string): boolean {
    const keyList = Object.keys(asyncStack);
    if (keyList.includes(str)) {
      const funcList = (asyncStack as Record<string, string[]>)[str];
      if (funcList.includes(symbol)) {
        return true;
      }
    }
    return false;
  ...
  private initData(node: ChartStruct, depth: number, calDisplay: boolean): void {
    ...
    node.isAsyncStack = this.isExpectedFunction(node.lib, splitSymbol(node));
    ...
    if (hoverNode) {
      ...
      switch (this._mode) {
        case ChartMode.Byte: {
          ...
          const counters = hoverNode.drawCount || hoverNode.count;
          this.hintContent = `
                    ...
                    <span class="bold">Count: </span> <span>${counters} (${countPercent}%)</span>`;
          break;
        }
        case ChartMode.Duration: {
        ...
        case ChartMode.Count: {
          ...
          this.hintContent = `
                      ...
                      <span class="bold">Addr: </span> <span>${hoverNode?.addr}</span> <br>
                      ${sourceHint}
                      <span class="bold">${label}: </span> <span class="count" jump> ${(count).toLocaleString()}</span>`;
          break;
        }
      }
  ...
  public initElements(): void {
    ...
    this.listenerResize();
    this.addEventListener('show-calibration', (e: Event) => {
      const customEvent = e as CustomEvent<{ isShow: boolean }>;
      this._isShow = customEvent.detail.isShow;
    });
  }
