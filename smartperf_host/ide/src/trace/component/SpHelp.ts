...
export class SpHelp extends BaseElement {
  ...
  private iframeMessageHandler: ((e: MessageEvent) => void) | undefined;
    ...
    mainMenu.menus = [
      {
        ...
        children: [
          ...
          this.setupGPUCountersMenu(that)
        ],
      },
      {
        collapsed: false,
  ...
  private setupMemoryMenu(that: this): MenuGroup {
    return {
      ...
      children: [
        ...
        {
          title: 'Gpu Memory抓取和展示说明',
          icon: '',
          clickHandler: function (item: MenuItem): void {
            that.handleMemoryMenuItemClick(that, 'native', 'quickstart_gpu_memory', '31');
          },
        },
        {
          title: 'FD & Thread抓取和展示说明',
          icon: '',
          clickHandler: function (item: MenuItem): void {
            that.handleMemoryMenuItemClick(that, 'native', 'quickstart_fd', '32');
          },
        },
        {
          title: '页内存抓取和展示说明',
  ...
  private setupIframeMessageListener(): void {
    ...
    const handler = (e: MessageEvent) => {
      if (e.data && e.data.type === 'navigate_help_doc' && e.data.index) {
        const targetDocName = `quickstart_${e.data.index === '27' ? 'extensions' : e.data.docName || ''}`;
        const targetEvent = e.data.event || 'extensions';
        SpStatisticsHttpUtil.addOrdinaryVisitAction({
          event: targetEvent,
          action: 'help_doc',
        });
        this.helpFile!.innerHTML = `<iframe id="myIframe" type="text/html" src='doc/${targetDocName}.html?${this.dark}' width="100%" height="100%"></iframe>`;
        this.navbarInit(targetDocName);
        this.changeItemURL(e.data.index);
      }
    };

    this.iframeMessageHandler = handler;
  ...
  private setupGPUCountersMenu(that: this): MenuGroup {
    return {
      collapsed: false,
      title: 'GPU Counters',
      describe: '',
      ...
      children: [
        {
          title: 'GPU Counters抓取和展示说明',
          icon: '',
          clickHandler: function (item: MenuItem): void {
            that.handleMemoryMenuItemClick(that, 'file_system', 'quickstart_gpu_counters', '32');
          },
        }
      ],
    };
