...
import { type TabPaneGMStatisticAnalysis } from '../sheet/gpu-memory/TabPaneGMStatisticAnalysis';
...
import { TabPaneSchedTaskChild } from '../sheet/cpu/TabPaneSchedTaskChild';
import { MemoryAnalysisDataLogic } from './MemoryAnalysisData';
import { MemoryTraceRowType } from '../../../bean/MemoryEnum';
import { IOStruct } from 'src/trace/database/ui-worker/ProcedureWorkerIO';
import { TabPaneIOStatistics } from '../sheet/io/TabPaneIOStatistics';
...
export class TraceSheet extends BaseElement {
  ...
  private rowClickEvent(): void {
    ...
    // @ts-ignore
    this.getComponentByID<unknown>('box-gpu-statistic-analysis')?.addEventListener('row-click', (e: MouseEvent) => {
      this.gpuAnalysisListener(e);
    });
    // @ts-ignore
    this.getComponentByID<unknown>('box-other-source-analysis')?.addEventListener('row-click', (e: MouseEvent) => {
      this.osAnalysisListener(e);
    });
    // @ts-ignore
    ...
    // @ts-ignore
    this.getComponentByID<unknown>('box-gpu-statstics')?.addEventListener('row-click', (e: unknown) => {
      this.gpuStatsticsListener(e);
    });
    // @ts-ignore
  ...
  private tdClickEvent(): void {
    // @ts-ignore
    this.getComponentByID<unknown>('box-sched-task')?.addEventListener('td-click', (evt: unknown) => {
      this.tdClickTaskHandler(evt);
    });
    // @ts-ignore
  ...
  private gpuAnalysisListener(e: MouseEvent): void {
    //@ts-ignore
    if (e.detail.button === 2 && e.detail.tableName) {
      let pane = this.getPaneByID('box-gpu-calltree');
      pane.hidden = false;
      this.litTabs!.activeByKey(pane.key);
    }
  }

  private osAnalysisListener(e: MouseEvent): void {
    //@ts-ignore
    if (e.detail.button === 2 && e.detail.tableName) {
      let pane = this.getPaneByID('box-os-calltree');
      pane.hidden = false;
      this.litTabs!.activeByKey(pane.key);
    }
  }
  ...
  private gpuStatsticsListener(e: unknown): void {
    // @ts-ignore
    if (e.detail.button === 0) {
      // @ts-ignore
      this.selection!.statisticsSelectData = e.detail;
      let pane = this.getPaneByID('box-gpu-memory');
      this.litTabs?.activeByKey(pane.key);
      // @ts-ignore
      (pane.children.item(0) as unknown)!.fromStastics(this.selection);
    }
  ...
  displayIoStatiicsData = (dataList: Array<IOStruct>): void => {
    this.displayTab<TabPaneIOStatistics>('box-io-statistics').setIoStatisticsData(dataList);
  };
  ...
  // 统计模式点选
  displayGenicMemoryData = (data: HeapStruct, childrenRowType: string, ipid: number, rowType: string): void => {
    let val = new SelectionParam();
    val.leftNs = data.startTime! + data.dur!;
    val.rightNs = data.startTime! + data.dur! + 1;
    this.selection = val;
    switch (rowType) {
      case TraceRow.ROW_TYPE_HEAP:
        val.nativeMemoryStatistic.push(childrenRowType);
        ...
        break;
      case TraceRow.ROW_TYPE_GPU_HEAP:
        val.gpuMemoryStatistic.push(childrenRowType);
        val.gpuMemoryCurrentIPid = ipid;
        val.gpuMemory = [];
        this.displayTab<TabPaneGMStatisticAnalysis>('box-gpu-statistic-analysis', 'box-gpu-calltree').data = val;

        break;
      case TraceRow.ROW_TYPE_OTHER_SOURCE_HEAP:
        val.otherSourceStatistic.push(childrenRowType);
        val.otherSourceCurrentIPid = ipid;
        val.otherSource = [];
        this.displayTab<TabPaneGMStatisticAnalysis>('box-other-source-analysis', 'box-os-calltree').data = val;
        break;
    }
  }
  ...
  updateRangeSelect(ipid?: number): boolean {
    if (
      ...
      (this.selection.nativeMemory.length > 0 ||
        ...
        this.selection.gpuMemory.length > 0 ||
        this.selection.gpuMemoryStatistic.length > 0 ||
        this.selection.otherSource.length > 0 ||
        this.selection.otherSourceStatistic.length > 0 ||
      ...
      const logic = new MemoryAnalysisDataLogic();
      if (param.nativeMemory.length > 0 || param.nativeMemoryStatistic.length > 0) {
        logic.setMemoryType(MemoryTraceRowType.ROW_TYPE_NATIVE_MEMORY);
        if (ipid) {
          logic.setCurrentSelectIPid(ipid);
          param.nativeMemoryCurrentIPid = ipid;
        }
      }
      if (param.gpuMemory.length > 0 || param.gpuMemoryStatistic.length > 0) {
        logic.setMemoryType(MemoryTraceRowType.ROW_TYPE_GPU_MEMORY);
        if (ipid) {
          logic.setCurrentSelectIPid(ipid);
          param.gpuMemoryCurrentIPid = ipid;
        }
      }
      if (param.otherSource.length > 0 || param.otherSourceStatistic.length > 0) {
        logic.setMemoryType(MemoryTraceRowType.ROW_TYPE_OTHER_SOURCE);
        if (ipid) {
          logic.setCurrentSelectIPid(ipid);
          param.otherSourceCurrentIPid = ipid;
        }
      }
  ...
  showUploadSoBt(selection: SelectionParam | null | undefined): void {
    if (
      ...
      (selection.nativeMemory.length > 0 ||
        ...
        selection.gpuMemory.length > 0 ||
        selection.gpuMemoryStatistic.length > 0 ||
        selection.otherSource.length > 0 ||
        selection.otherSourceStatistic.length > 0 ||
  ...
  tdClickTaskHandler(e: unknown): void {
    // @ts-ignore
    ...
    let pane = this.getPaneByID('box-schedtask-child'); //通过Id找到需要展示的Tab页
    ...
    if (e.detail.tid) {
      // @ts-ignore
      pane.tab = Utils.getInstance().getThreadMap().get(Number(e.detail.tid)) ? `${Utils.getInstance().getThreadMap().get(Number(e.detail.tid))}[${e.detail.tid}]` : `Thread[${e.detail.tid}]`;
    } else {
      // @ts-ignore
      pane.tab = Utils.getInstance().getProcessMap().get(Number(e.detail.pid)) ? `${Utils.getInstance().getProcessMap().get(Number(e.detail.pid))}[${Number(e.detail.pid)}]` : `Process[${e.detail.pid}]`;
    }
    ...
    // @ts-ignore
    param.threadId = e.detail.tid ? e.detail.tid : null;
    param.isJumpPage = true;
    (pane.children.item(0) as TabPaneSchedTaskChild).data = param;
  }
