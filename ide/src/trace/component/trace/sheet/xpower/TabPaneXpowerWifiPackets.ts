...
@element('tabpane-xpower-wifipackets')
export class TabPaneXpowerWifiPackets extends BaseElement {
  private xpowerCounterTbl: LitTable | null | undefined;
  private xpowerCounterRange: HTMLLabelElement | null | undefined;
  private xpowerPacketsSource: Array<SelectionData> = [];
  ...
  async getCounterData(xpowerCounterValue: SelectionParam): Promise<void> {
    let collect = xpowerCounterValue.xpowerWifiPacketsMapData;
    this.xpowerCounterTbl!.loading = true;
    let dataSourcePackets: Array<SelectionData> = [];
    for (let key of collect.keys()) {
      ...
      this.checkedValue.forEach((key) => {
        ...
        res!.forEach((item) => {
          // @ts-ignore
          let value = item[key];
          if (xpowerWifiMap.has(key) && value !== 0) {
      ...
      xpowerWifiMap.forEach((value, mapKey) => {
        ...
        dataSourcePackets.push(sd);
      });
      this.xpowerCounterTbl!.loading = false;
      this.xpowerPacketsSource = dataSourcePackets;

      this.xpowerCounterTbl!.recycleDataSource = dataSourcePackets;
    }
  }
  ...
  sortByColumn(detail: { key: string; sort: number }): void {
    ...
    if (detail.key === 'name') {
      this.xpowerPacketsSource.sort(compare(detail.key, detail.sort, 'string'));
    } else {
      this.xpowerPacketsSource.sort(compare(detail.key, detail.sort, 'number'));
    }
    this.xpowerCounterTbl!.recycleDataSource = this.xpowerPacketsSource;
  }
