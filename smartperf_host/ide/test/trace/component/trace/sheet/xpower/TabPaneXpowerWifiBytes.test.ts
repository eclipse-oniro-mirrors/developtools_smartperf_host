...
import { TabPaneXpowerWifiBytes } from '../../../../../../src/trace/component/trace/sheet/xpower/TabPaneXpowerWifiBytes';
...
describe('TabPaneXpowerWifiBytes', () => {
  let tabpane = new TabPaneXpowerWifiBytes();
  let selectionParam = new SelectionParam();
  let traceRow = new TraceRow<XpowerWifiStruct>();
  traceRow.rowId = 'WIFIBytes';
  selectionParam.xpowerStatisticMapData.set(
    ...
    (args: unknown): Promise<XpowerWifiStruct[]> | undefined => {
      ...
      result = xpowerWifiDataSender(traceRow, 'WIFIBytes', args);
      return result;
    }
  );
  ...
  it('TabPaneXpowerWifiBytesTest01', () => {
    let list = [10, 30, 20];
    ...
    let sd = tabpane.createSelectCounterData(list, 'tx', 'tx');
