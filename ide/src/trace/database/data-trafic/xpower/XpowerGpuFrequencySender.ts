...
import { XpowerGpuFreqCountStruct } from '../../ui-worker/ProcedureWorkerXpowerGpuFreqCount';
import { XpowerGpuFreqStruct } from '../../ui-worker/ProcedureWorkerXpowerGpuFreq';
import { Utils } from '../../../component/trace/base/Utils';

export function xpowerGpuFreqCountDataSender(
  row: TraceRow<XpowerGpuFreqCountStruct>,
  args?: unknown
): Promise<XpowerGpuFreqCountStruct[]> {
  ...
  return new Promise((resolve, reject): void => {
    getThreadPool(row.traceId).submitProto(
      QueryEnum.XpowerGpuFreqCountData,
      {
...
function gpuFreqCountArrayBufferHandler(buffers: unknown, len: number): XpowerGpuFreqCountStruct[] {
  ...
  for (let i = 0; i < len; i++) {
    outArr.push({
      ...
      startNS: startNS[i],
      dur: dur[i],
    } as unknown as XpowerGpuFreqCountStruct);
  }
  return outArr;
}

export function xpowerGpuFreqDataSender(
  row: TraceRow<XpowerGpuFreqStruct>,
  args?: unknown
): Promise<XpowerGpuFreqStruct[]> {
  let trafic: number = TraficEnum.Memory;
  let width = row.clientWidth - CHART_OFFSET_LEFT;
  if (trafic === TraficEnum.SharedArrayBuffer && !row.sharedArrayBuffers) {
    row.sharedArrayBuffers = {
      startNS: new SharedArrayBuffer(Float64Array.BYTES_PER_ELEMENT * MAX_COUNT),
      dur: new SharedArrayBuffer(Float64Array.BYTES_PER_ELEMENT * MAX_COUNT),
      runTime: new SharedArrayBuffer(Float64Array.BYTES_PER_ELEMENT * MAX_COUNT),
      idleTime: new SharedArrayBuffer(Float64Array.BYTES_PER_ELEMENT * MAX_COUNT),
      frequency: new SharedArrayBuffer(Float64Array.BYTES_PER_ELEMENT * MAX_COUNT),
    };
  }
  return new Promise((resolve, reject): void => {
    getThreadPool(row.traceId).submitProto(
      QueryEnum.XpowerGpuFreqData,
      {
        startNS: TraceRow.range?.startNS || 0,
        endNS: TraceRow.range?.endNS || 0,
        totalNS: TraceRow.range?.totalNS || 0,
      ...
      (res: unknown, len: number, transfer: boolean): void => {
        resolve(gpuFreqArrayBufferHandler(transfer ? res : row.sharedArrayBuffers, len));
      }
    );
  });
}

function gpuFreqArrayBufferHandler(buffers: unknown, len: number): XpowerGpuFreqStruct[] {
  let outArr: XpowerGpuFreqStruct[] = [];
  // @ts-ignore
  let startNS = new Float64Array(buffers.startNS);
  // @ts-ignore
  let dur = new Float64Array(buffers.dur);
  // @ts-ignore
  let runTime = new Float64Array(buffers.runTime);
  // @ts-ignore
  let idleTime = new Float64Array(buffers.idleTime);
  // @ts-ignore
  let frequency = new Float64Array(buffers.frequency);
  for (let i = 0; i < len; i++) {
    outArr.push({
      startNS: startNS[i],
      dur: dur[i],
    ...
    } as unknown as XpowerGpuFreqStruct);
  }
  return outArr;
}
