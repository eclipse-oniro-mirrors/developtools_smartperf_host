...
import { MemoryBasicType, MemoryStatisticType, MemoryTableName, MemoryType } from "../../../../bean/MemoryEnum";
...
export class TabpaneOSCalltree extends BaseElement {
  ...
  private filterTypes: Array<string> = [...MemoryBasicType.OTHER_SOURCE];
  ...
  set data(osCallTreeParam: SelectionParam) {
    this.isStatisticMode = osCallTreeParam.otherSourceStatistic.length > 0;
    this.osCallTreeFrameChart!.dispatchEvent(
      new CustomEvent('show-calibration', {
        detail: {
          isShow: !this.isStatisticMode
        },
        bubbles: false
      })
    );
  ...
  private async init(osCallTreeParam: SelectionParam): Promise<void> {
    // 初始化样式
    this.initUI(osCallTreeParam);
    // 初始化选择框内容
    // 根据 isStatistic 设置 isStatistics 属性
    if (this.isStatisticMode) {
      this.osCallTreeFilter!.setAttribute('isStatistics', '');
    } else {
      this.osCallTreeFilter!.removeAttribute('isStatistics');
    }
    let types: Array<string | number> = [];
    this.initTypes(osCallTreeParam, types);
    await this.initFilterTypes(osCallTreeParam, types);
    ...
    this.getDataByWorkerQuery(
      ...
      (results: unknown[]): void => {
        ...
        if (this._initFromAnalysis) {
          ...
          this.osCallTreeFrameChart!.totalRootData = this.osCallTreeSource as Array<ChartStruct>;
          this.osCallTreeFrameChart!.data = this.osCallTreeSource as Array<ChartStruct>;
  ...
  initUI(osCallTreeParam: SelectionParam): void {
    this.currentSelection = osCallTreeParam;
    this.headLine!.clear();
    ...
    procedurePool.submitWithName('logic0', 'otherSource-reset', [], undefined, () => { });
  ...
  initElements(): void {
    ...
    this.osCallTreeFrameChart!.mode = ChartMode.Count;
  ...
  private initCloseCallBackByHeadLine(): void {
    ...
    this.headLine!.closeCallback = (): void => {
      ...
      this.initTypes(this.currentSelection!, types);
      this.initFilterTypes(this.currentSelection!, types);
  ...
  private initGetFilterByOSCallTreeFilter(osCallTreeData: FilterData): void {
    const typeName = this.typeIndexToTypeName(osCallTreeData.secondSelect)
    if (
      this.filterLifespanType !== osCallTreeData.firstSelect ||
      this.filterOtherSourceType !== typeName ||
  ...
  private setGroupArgsByRefreshAllNode(): Map<string, unknown> {
    ...
    groupArgs.set(
      'nativeHookType',
      this.currentSelection!.otherSource.length > 0 ? 'normal' : 'statistic'
    );
    return groupArgs;
  }
  ...
  private filterFuncByButtonType(osCallTreeFuncData: unknown, osCallTreeFuncArgs: unknown[]): unknown[] {
    ...
    if (osCallTreeFuncData.item === 'symbol') {
      ...
      if (this.currentSelectedData !== undefined && this.currentSelectedData.lib !== '') {
        // @ts-ignore
        this.osCallTreeFilter!.addDataMining({ name: this.currentSelectedData.lib }, osCallTreeFuncData.item);
        osCallTreeFuncArgs.push({
          funcName: 'splitTree',
          // @ts-ignore
  ...
  async initFilterTypes(osCallTreeParam: SelectionParam, types: Array<string | number>): Promise<void> {
    this.currentOSCallTreeFilter = this.shadowRoot?.querySelector<TabPaneFilter>('#os-call-tree-filter');
    this.filterTypes = [...MemoryBasicType.OTHER_SOURCE];
    const tableName = this.isStatisticMode ? MemoryTableName.NATIVE_HOOK_STATISTIC : MemoryTableName.NATIVE_HOOK;

    await querySubType(osCallTreeParam.leftNs, osCallTreeParam.rightNs, osCallTreeParam.otherSourceCurrentIPid,
      tableName, types).then((res: { subTypeId: number, subType: string, eventType: string }[]) => {
        this.filterTypes.push(...res.map((item) => {
          if (this.isStatisticMode) {
            if (item.subType === null || item.subType.startsWith('Other ')) {
              const type = parseInt(item.eventType)
              if (type === MemoryStatisticType.FD) {
                return `Other ${MemoryType.FD_OPEN_NAME}`
              } else if (type === MemoryStatisticType.THREAD) {
                return `Other ${MemoryType.THREAD_CREATE_NAME}`
              }
            }
          }
          return item.subType;
    ...
    this.oSCallTreeFilterExtend(this.filterTypes, osCallTreeParam);
  }

  private oSCallTreeFilterExtend(secondFilterList: string[], osCallTreeParam: SelectionParam): void {
    procedurePool.submitWithName('logic0', 'otherSource-get-responseType', {}, undefined, (res: unknown) => {
      // @ts-ignore
  ...
  initTypes(osCallTreeParam: SelectionParam, types: Array<string | number>): void {
    // normal 模式
    if (!this.isStatisticMode) {
      const otherSourceSet = new Set(osCallTreeParam.otherSource);
      if (otherSourceSet.has(MemoryBasicType.OTHER_SOURCE[1])) {
        types.push(`${MemoryType.FD_OPEN_NAME}`);
      }
      if (otherSourceSet.has(MemoryBasicType.OTHER_SOURCE[2])) {
        types.push(`${MemoryType.THREAD_CREATE_NAME}`);
      }
    } else {
      const otherSourceStatisticSet = new Set(osCallTreeParam.otherSourceStatistic);
      if (otherSourceStatisticSet.has(MemoryBasicType.OTHER_SOURCE[1])) {
        types.push(MemoryStatisticType.FD);
      }
      if (otherSourceStatisticSet.has(MemoryBasicType.OTHER_SOURCE[2])) {
        types.push(MemoryStatisticType.THREAD);
      }
    }
  }
  ...
  getDataByWorkerQuery(args: unknown, handler: Function): void {
    ...
    procedurePool.submitWithName(
      ...
      this.currentSelection!.otherSource!.length > 0
        ? 'otherSource-queryCallchainsSamples'
        : 'otherSource-queryStatisticCallchainsSamples',
      args,
      undefined,
