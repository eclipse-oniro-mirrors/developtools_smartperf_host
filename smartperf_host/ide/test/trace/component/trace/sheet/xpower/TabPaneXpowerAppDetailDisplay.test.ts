...
import { TabPaneXpowerDisplay } from '../../../../../../src/trace/component/trace/sheet/xpower/TabPaneXpowerAppDetailDisplay';
...
describe('TabPaneXpowerDisplay', () => {
  let tabpane = new TabPaneXpowerDisplay();
  let selectionParam = new SelectionParam();
  let traceRow = new TraceRow<XpowerAppDetailStruct>();
  traceRow.rowId = 'AppDetailDisplay';
  selectionParam.xpowerDisplayMapData.set(
    traceRow.rowId,
    (args: unknown): Promise<XpowerAppDetailStruct[]> | undefined => {
  ...
  it('TabPaneXpowerDisplayTest01', () => {
    ...
    let sd = tabpane.createSelectCounterData(list, 'c10hz');
    expect(sd.maxNumber).toEqual(30);
    expect(sd.minNumber).toEqual(10);
    expect(sd.avgNumber).toEqual(20);
  });
});
