...
import { SpGpuCountersRecord } from './setting/SpGpuCountersRecord'
...
export class SpRecordTrace extends BaseElement {
  ...
  private spGpuCounters: SpGpuCountersRecord | undefined;
  ...
  private initConfigPage(): void {
    ...
    this.spGpuCounters = new SpGpuCountersRecord();
  ...
  private buildNormalTraceItem(): void {
    this._menuItems = [
      ...
      this.buildMenuItem('GpuCounters', 'externaltools', this.spGpuCounters!)
    ];
  }
  ...
  private recordTraceCmd(traceCommandStr: string): void {
    ...
    if (this.spGpuCounters?.recordCpuCounters) {
      setTimeout(() => {
        let period = this.spGpuCounters?.periodCurrentValue;
        if (Number(period) > 1000 || period === '') {
          period = '100';
        }
        HdcDeviceManager.shellResultAsString(CmdConstant.CMD_SHELL + 'counters_gather -p ' + period, false).then((): void => { });
      }, 1);
    }
  }

  private recordLongTraceCmd(traceCommandStr: string): void {
    HdcDeviceManager.shellResultAsString(CmdConstant.CMD_CLEAR_LONG_FILE, false).then((res) => {
      HdcDeviceManager.shellResultAsString(CmdConstant.CMD_SHELL + traceCommandStr, false).then((traceResult) => {
        ...
        if (re === 0) {
          ...
          HdcDeviceManager.shellResultAsString(
            CmdConstant.CMD_TRACE_FILE_SIZE + this.recordSetting!.longOutPath + 'hiprofiler_data_*',
            false
          ).then((traceFileSize) => {
            this.litSearch!.setPercent(`traceFileSize is ${traceFileSize}`, -1);
            if (traceFileSize.indexOf('No such') !== -1) {
  ...
  private recordLongTrace(): void {
    ...
    HdcDeviceManager.shellResultAsString(CmdConstant.CMD_GET_LONG_FILES + this.recordSetting!.longOutPath + 'hiprofiler_data_*', false).then(
      (result) => {
